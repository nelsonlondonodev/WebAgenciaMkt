...
- **Refactorización Premium del Menú Móvil**:
  - **Overlay Inteligente**: Implementación de un sistema de oscurecimiento dinámico con `backdrop-blur-sm` y sincronización de opacidad para una transición fluida.
  - **Bloqueo de Scroll Universal**: Mejora de la clase `.menu-open` (ahora aplicada a `html` y `body`) para eliminar el scroll de fondo y los saltos visuales en móviles.
  - **Accesibilidad Avanzada**: Soporte nativo para cierre con tecla `Esc` y actualización de atributos `aria-expanded` y `aria-hidden` para una navegación inclusiva.
  - **Navegación Táctica (ScrollSpy)**: Los enlaces del menú móvil ahora resaltan automáticamente la página actual o la sección activa del portafolio mediante la clase `.mobile-link-active`.
- **Automatización de Despliegue**:
  - **Build v.2026.04.30.1349**: Sincronización masiva de todos los archivos HTML con el nuevo sistema de versionado de caché para forzar la actualización inmediata en el navegador del cliente.
  - **Limpieza de IDs**: Estandarización de `z-index: 10001` para el menú móvil, garantizando su prioridad visual sobre chatbots y otros elementos interactivos.
...
- **Footer Visualmente Superior:**
  ...
  - **Efectos Glow en RRSS:** Los iconos sociales ahora cuentan con un efecto de resplandor (_glow_) reactivo al hover, elevando la interactividad.
...
- **Autoría y Prueba Social:**
  - Actualización del contador de reseñas de Google Maps a **20 reseñas reales (5.0 Rating)**.
...
- **SEO de "Reloj Suizo":**
  - **Schema Markup:** Enriquecimiento de datos estructurados con `AggregateRating` (5.0/20 reseñas) y expansión de `areaServed` a nivel local (Fuente el Saz, Algete, Madrid).
...
- **Humanización de Marca:** Rediseño integral de la Hero Section en `index.html` con un layout de 2 columnas (50/50). Integración de la imagen personal de Nelson Londoño con efectos de _glow_ perimetral y máscara de degradado circular para una fusión orgánica con el fondo, ocultando rastros de edición por IA.
- **Social Proof Estratégico:** Inclusión de un Trust Badge de Google Reviews con identidad visual corporativa (colores oficiales), resaltando la calificación de 5.0 y 20 reseñas reales para aumentar la credibilidad instantánea.
...
- **Guía Principal (Hero):**: _5 Errores Fatales en Google Maps_. Apunta al documento `Google_Maps_Dominance_compressed.pdf`.
- **Recursos Secundarios (Grid):**
  - _Guía de Automatización (n8n)_: Hoja de ruta para aprender automatización (`guia-n8n.pdf`).
  - _Guía SEO Local (Jardinería Digital)_: Estrategias de posicionamiento (`Jardinería_Digital_SEO_Local_2026.pdf`).

_Importante: Los componentes `nav.html` y `footer.html` utilizan rutas absolutas (`/image/...` o `/blog.html`) para garantizar su correcta visualización desde subcarpetas como `/guia/`. Todos los recursos están bajo estrictas políticas `noindex, nofollow` para evitar que los buscadores los listen públicamente._
