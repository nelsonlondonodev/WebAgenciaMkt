(()=>{function I(){let s=document.getElementById("darkModeToggle");if(s){let e=["auto","light","dark"],t=localStorage.getItem("theme")||"auto",i=r=>{if(r==="auto"){let o=window.matchMedia("(prefers-color-scheme: dark)").matches;document.documentElement.classList.toggle("dark",o)}else document.documentElement.classList.toggle("dark",r==="dark")},a=r=>{let o=s.querySelector("i");o&&(o.classList.remove("fa-sun","fa-moon","fa-circle-half-stroke"),r==="light"?o.classList.add("fa-sun"):r==="dark"?o.classList.add("fa-moon"):o.classList.add("fa-circle-half-stroke"))};s.addEventListener("click",()=>{let o=(e.indexOf(t)+1)%e.length;t=e[o],localStorage.setItem("theme",t),i(t),a(t)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",()=>{t==="auto"&&i("auto")}),i(t),a(t)}}function L(){document.body.classList.add("fade-in");function s(t){let i=document.querySelector(t);if(i){let a=document.getElementById("mainHeader")?.offsetHeight||80,o=i.getBoundingClientRect().top+window.pageYOffset-a;window.scrollTo({top:o,behavior:"smooth"})}}function e(t,i){document.body.classList.add("fade-out"),setTimeout(()=>{i?(s(t),setTimeout(()=>{document.body.classList.remove("fade-out")},50)):window.location.href=t},300)}document.querySelectorAll('a:not([href^="tel:"]):not([href^="mailto:"]):not(.social-share-btn):not(.modal-contact-button):not([target="_blank"])').forEach(t=>{t.addEventListener("click",function(i){let a=this.getAttribute("href");if(!a||a==="#")return;let r=a.startsWith("#"),o=r||a.split("#")[0]===window.location.pathname.split("/").pop()||a.split("#")[0]==="";(this.hostname===window.location.hostname||a.startsWith("/"))&&(i.preventDefault(),e(a,r&&o))})})}function k(){let s=document.querySelectorAll(".scroll-animate-initial");if(s.length>0&&typeof IntersectionObserver<"u"){let e=new IntersectionObserver(t=>{t.forEach(i=>{i.isIntersecting&&(i.target.classList.add("is-visible"),e.unobserve(i.target))})},{threshold:.1});s.forEach((t,i)=>{t.style.transitionDelay=`${30*i}ms`,e.observe(t)})}}function q(){let s=document.createElement("div");return s.id="nav-overlay",s.className="fixed inset-0 bg-black/50 z-10 hidden",document.body.appendChild(s),s}function A(){let s=document.getElementById("mobileMenuButton"),e=document.getElementById("mobileMenu"),t=q();if(!s||!e)return;let i=()=>{e.classList.add("hidden"),t.classList.add("hidden"),s.classList.remove("is-active")},a=()=>{e.classList.remove("hidden"),t.classList.remove("hidden"),s.classList.add("is-active")},r=()=>{e.classList.contains("hidden")?a():i()};s.addEventListener("click",o=>{o.stopPropagation(),r()}),t.addEventListener("click",i),e.addEventListener("click",o=>{o.target.closest("a")&&(o.target.closest("#mobileServicesButton")||i())})}function O(){let s=document.getElementById("mobileServicesButton"),e=document.getElementById("mobileServicesMenu");!s||!e||s.addEventListener("click",t=>{t.stopPropagation(),e.classList.toggle("hidden");let i=s.querySelector("i.fa-chevron-down");i&&i.classList.toggle("rotate-180")})}function H(){let s=document.querySelectorAll("section[id]"),e=document.querySelectorAll("header nav a");if(s.length===0||e.length===0)return;let t={root:null,rootMargin:"0px 0px -60% 0px",threshold:0},i=new IntersectionObserver(a=>{a.forEach(r=>{if(r.isIntersecting){let o=r.target.id;e.forEach(n=>{n.classList.remove("nav-active"),n.getAttribute("href")===`#${o}`&&n.classList.add("nav-active")})}})},t);s.forEach(a=>{i.observe(a)})}function D(){let s=document.getElementById("services-menu-group"),e=document.getElementById("services-menu-dropdown"),t;!s||!e||(s.addEventListener("mouseenter",()=>{clearTimeout(t),e.classList.remove("hidden")}),s.addEventListener("mouseleave",()=>{t=setTimeout(()=>{e.classList.add("hidden")},300)}))}function x(){A(),O(),H(),D()}function w(){let s=document.getElementById("portfolio-filters"),e=document.querySelectorAll("#portfolio-grid .portfolio-item");s&&e.length>0&&s.addEventListener("click",t=>{if(t.target.matches("button.filter-btn")){let i=t.target;if(i.classList.contains("active-filter"))return;s.querySelector(".active-filter").classList.remove("active-filter"),i.classList.add("active-filter");let a=i.dataset.filter;e.forEach(r=>{let o=r.dataset.category,n=a==="all"||o.includes(a);r.style.display=n?"block":"none"})}})}function P(s,e,t,i,a=0){let r=null;function o(n){r===null&&(r=n);let c=n-r,d=Math.min(c/i,1),l=e<t,m=t-e,f=e+m*d;d===1?s.textContent=parseFloat(t).toFixed(a):s.textContent=f.toFixed(a),d<1&&requestAnimationFrame(o)}requestAnimationFrame(o)}function S(){let s=document.querySelectorAll(".animate-number");if(s.length===0)return;let e=new IntersectionObserver((t,i)=>{t.forEach(a=>{if(a.isIntersecting){let r=a.target,o=parseFloat(r.dataset.endValue),n=parseFloat(r.dataset.startValue)||0,c=parseInt(r.dataset.duration,10)||2e3,d=parseInt(r.dataset.decimals,10)||0;if(isNaN(o)){console.error("El atributo data-end-value no es un n\xFAmero v\xE1lido.",r);return}P(r,n,o,c,d),i.unobserve(r)}})},{threshold:.5});s.forEach(t=>{e.observe(t)})}var h=async(s,e)=>{try{let t=await fetch(e);if(!t.ok)throw new Error(`Error al cargar el componente ${e}: ${t.statusText}`);let i=await t.text(),a=document.querySelector(s);a?a.innerHTML=i:console.warn(`No se encontr\xF3 el elemento con el selector: ${s}`)}catch(t){console.error(`No se pudo cargar el componente desde ${e}:`,t)}},B=async()=>{await Promise.all([h("#nav-placeholder","/components/nav.html"),h("#footer-placeholder","/components/footer.html"),h("#cookie-banner-placeholder","/components/cookie-banner.html")]),document.querySelector('[data-component="chatbot"]')&&await h('[data-component="chatbot"]',"/components/chatbot.html")};var M=()=>{let s=document.getElementById("currentYear");s&&(s.textContent=new Date().getFullYear())};var v=class{constructor(){this.elements={widgetButton:document.getElementById("chat-widget-button"),window:document.getElementById("chat-window"),closeButton:document.getElementById("close-chat-window"),sendButton:document.getElementById("chat-send"),input:document.getElementById("chat-input"),messages:document.getElementById("chat-messages")},this.elements.widgetButton&&(this.n8nWebhookUrl="https://n8n.srv1033442.hstgr.cloud/webhook/34b5ab96-ecf0-4195-93de-e3923c2062e5",this.sessionIdKey="nelson_chat_session_id",this.sessionId=this.getOrCreateSessionId(),this.historyKey=`nelson_chat_history_${this.sessionId}`,this.history=[],this.loadHistory(),this.addEventListeners())}getOrCreateSessionId(){let e=localStorage.getItem(this.sessionIdKey);return e||(e=Date.now().toString(36)+Math.random().toString(36).substring(2),localStorage.setItem(this.sessionIdKey,e)),e}loadHistory(){let e=sessionStorage.getItem(this.historyKey);e&&(this.history=JSON.parse(e),this.history.forEach(t=>{this.appendMessage(t.message,t.sender,!1)}))}addEventListeners(){this.elements.widgetButton.addEventListener("click",()=>this.toggleWindow()),this.elements.closeButton.addEventListener("click",()=>this.toggleWindow()),this.elements.sendButton.addEventListener("click",()=>this.sendMessage()),this.elements.input.addEventListener("keypress",e=>{e.key==="Enter"&&this.sendMessage()})}toggleWindow(){this.elements.window.classList.toggle("hidden"),this.elements.window.classList.contains("hidden")||(this.elements.input.focus(),this.history.length===0&&this.appendMessage("\xA1Hola! Soy Aurelio, tu asistente virtual. \xBFEn qu\xE9 puedo ayudarte?","bot"))}sendMessage(){let e=this.elements.input.value.trim();e!==""&&(this.appendMessage(e,"user"),this.elements.input.value="",this.showTypingIndicator(),this.sendToWebhook(e,this.sessionId))}async sendToWebhook(e,t){try{let a=await(await fetch(this.n8nWebhookUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:e,sessionId:t})})).json();this.removeTypingIndicator();let r=a.response||"Lo siento, no he podido procesar tu mensaje. Por favor, intenta de nuevo.";this.appendMessage(r,"bot")}catch(i){console.error("Error sending message to n8n:",i),this.removeTypingIndicator(),this.appendMessage("Lo siento, ha ocurrido un error de conexi\xF3n. Por favor, int\xE9ntalo m\xE1s tarde.","bot")}}createBubble(e,t){let i=document.createElement("div");return i.className=`py-2 px-4 inline-block max-w-xs ${t==="user"?"bg-primary-blue text-white rounded-t-lg rounded-bl-lg":"bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white rounded-t-lg rounded-br-lg"}`,i.innerHTML=e,i.querySelectorAll("a").forEach(a=>a.classList.add("text-blue-500","underline")),i}createTimestamp(){let e=new Date().toLocaleTimeString("es-ES",{hour:"2-digit",minute:"2-digit"}),t=document.createElement("div");return t.className="text-xs text-gray-500 dark:text-gray-400 mt-1",t.textContent=e,t}appendMessage(e,t,i=!0){i&&(this.history.push({message:e,sender:t}),sessionStorage.setItem(this.historyKey,JSON.stringify(this.history)));let a=document.createElement("div");a.className=`mb-4 flex flex-col ${t==="user"?"items-end":"items-start"} animate-chat-message-in`;let r=this.createBubble(e,t),o=this.createTimestamp();a.appendChild(r),a.appendChild(o),this.elements.messages.appendChild(a),this.elements.messages.scrollTop=this.elements.messages.scrollHeight}showTypingIndicator(){let e=document.createElement("div");e.id="typing-indicator",e.className="mb-4",e.innerHTML=`
